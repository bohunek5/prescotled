...
export async function mountAssemblyPreview(host,button,caption,{auto=false}={}){
 ...
 try{
  ...
  film=await createWelcomeFilm(host,{onPhase:phase=>caption.textContent=captions[phase]||captions.tape,onPlaying:playing=>{button.textContent=playing?'Ⅱ  Pauza':'▷  Odtwórz pokaz';button.setAttribute('aria-pressed',String(playing));}});
