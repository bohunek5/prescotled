(function(){if(!(document.currentScript instanceof HTMLScriptElement))return;
    ...
    if(url_structure == 'none') {
        ...
        function load_tlib(){if(!window.PrescotPrivacy?.has('translation'))return;if(!window.gt_translate_script){window.gt_translate_script=document.createElement('script');gt_translate_script.src='https://translate.google.com/translate_a/element.js?cb=googleTranslateElementInit2';document.body.appendChild(gt_translate_script);}}
        window.doGTranslate = function(lang_pair) {
            if(lang_pair && lang_pair.value) lang_pair=lang_pair.value;
            if(typeof lang_pair !== 'string' || !lang_pair.includes('|')) return;
            var lang=lang_pair.split('|')[1];
            if(lang===default_language) {
                if(!get_current_lang() || get_current_lang()===default_language) return;
            } else if(!window.PrescotPrivacy?.has('translation')) {
                window.PrescotPrivacy?.request('translation',function(){window.doGTranslate(lang_pair)});
                return;
            }
            load_tlib();
            var attempts=0;
            function applyLanguage() {
                if(!window.PrescotPrivacy?.has('translation')) return;
                var combo=document.querySelector('select.goog-te-combo');
                if(!combo || !combo.options.length) {
                    if(++attempts<30) setTimeout(applyLanguage,400);
                    return;
                }
                combo.value=lang;fire_event(combo,'change');
            }
            applyLanguage();
        };
        ...
        if(current_lang != default_language)
            ...
            void 0; // Translation starts only after selecting a language.
    }
    ...
    if(window.PrescotPrivacy?.has('translation') && detect_browser_language && window.localStorage && window.navigator && localStorage.getItem('gt_autoswitch') == null && !/bot|spider|slurp|facebook/i.test(navigator.userAgent)) {
