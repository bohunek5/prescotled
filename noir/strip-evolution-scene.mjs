...
export function createEvolutionScene(host,{presentation=false}={}){
 ...
 vec3 evoLight(float u){return mix(uEvoA,uEvoB,evoIncoming(u));}
 `;
 ...
 const printMaterials=[],printTextures={day:[],night:[]};
 ...
 function createPack(family,density,cutOverride){
  ...
  const rgb=family==='rgb',cob=family.startsWith('cob'),slim=family==='slim',sshape=family==='sshape',pitch=100/density,count=Math.floor(length/pitch),components=Array.from({length:count},(_,i)=>({u:sshape?(i+.5)*pitch/length:along((i+.5)*pitch)}));
  ...
  if(cob){
   ...
   const w=rgb?.5:.28,h=rgb?.5:.35;
   instances(group,new T.BoxGeometry(w,h,rgb?.095:.062),body,components.map(c=>({...c,z:.04})));
   instances(group,new T.PlaneGeometry(rgb?.345:.175,rgb?.345:.228),emitter,components.map(c=>({...c,z:rgb?.093:.075}))).renderOrder=3;
 ...
 createPack('delux3',160);createPack('smd',128);createPack('slim',160);createPack('sshape',60);createPack('rgb',60);createPack('cob',528);createPack('smd5',60,1.7);createPack('smd12',60,5);createPack('smd24',120,5);createPack('cob48',480,5);
 ...
 function setTheme(next){
  ...
  printMaterials.forEach(({material,kind})=>{if(printTextures[presentation?'day':theme][kind]){material.map=printTextures[presentation?'day':theme][kind];material.opacity=.95;material.needsUpdate=true;}});
 ...
 function update(index,progress,pointerX=0,pointerY=0,seconds=0){
  if(disposed)return;resize();if(index!==lastIndex||progress!==lastProgress)lastAura=-1;lastIndex=index;lastProgress=progress;
  const day=theme==='day',state=evolutionState(progress),a=models[state.from],b=models[state.to],blend=state.blend;
  ...
  uniforms.uEvoA.value.copy(colors[state.from]);uniforms.uEvoB.value.copy(colors[state.to]);
